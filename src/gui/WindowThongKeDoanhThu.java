...
public class WindowThongKeDoanhThu extends JPanel{
   ...
   public void btXemActionPerformed(){
        int thang = Integer.parseInt(cbThang.getSelectedItem().toString().trim());
        int nam = Integer.parseInt(cbNam.getSelectedItem().toString().trim());
        
        try {
            ThongKeDoanhThuDAO dao = new ThongKeDoanhThuDAO();
            ThongKeResult result = dao.thongKeDoanhThu(thang, nam);

            int soLuongVe = result.getSoLuongVe();
            double tongTien = result.getTongTien();

            WindowThongTinThongKe w = new WindowThongTinThongKe();
            w.setThongTin(thang, nam, soLuongVe, tongTien);
            w.setVisible(true);
